...
namespace QuickGraph_Tests_01
{
    ...
    public class UnitTest1
    {
        ...
        public void TestMethod1()
        {
            ...
            Func<Edge<string>, double> constantCost = e => 1; // constant cost
            // We want to use Dijkstra on this graph
            DijkstraShortestPathAlgorithm<string, Edge<string>> dijkstra = new DijkstraShortestPathAlgorithm<string, Edge<string>>(graph, (key) => edgeCost[key]);//constantCost);

            // attach a distance observer to give us the shortest path distances
            VertexDistanceRecorderObserver<string, Edge<string>> distObserver = new VertexDistanceRecorderObserver<string, Edge<string>>((key) => edgeCost[key]);//constantCost);
